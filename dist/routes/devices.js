...
const normalizeMac = (macAddress) => macAddress.trim().toUpperCase().replace(/-/g, ':');
// GET /api/devices
...
router.post('/heartbeat', async (req, res) => {
    ...
    try {
        ...
        let device = await prisma_1.default.device.findUnique({ where: { macAddress: cleanMac } });
        ...
        if (!device) {
            ...
            if (targetUserId !== 'system' && device.userId !== targetUserId) {
                ...
                await prisma_1.default.device.update({ where: { id: device.id }, data: { userId: targetUserId } });
